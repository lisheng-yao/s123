...
//        	String json="";
			...
			svc.addCarousel(shopCarouselTitle, shopCarouselText, shopCarouselPic, shopCarouselStartTime,
		...
		if (("json2".equals(dataTypeAction) && contentType != null && contentType.startsWith("application/json"))) {
			...
			if (base64Image.isEmpty()) {
				ShopCarouselService svc = new ShopCarouselService();
				shopCarouselPic = svc.getOneShopCarousel(shopCarouselNo).getShopCarouselPic();
			} else {
				// Base64字串中移除資料類型的部分 data:image/jpeg;base64,/!!!
				base64Image = base64Image.substring(base64Image.indexOf(",") + 1);
			...
			ShopCarouselService svc = new ShopCarouselService();
			svc.updateShopCarousel(shopCarouselNo, shopCarouselTitle, shopCarouselText, shopCarouselPic,
